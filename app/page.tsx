...
export default function Home() {
  ...
  const [wallpaperOrder, setWallpaperOrder] = useState<number[]>([]);
  ...
  const [artistOnlySearch, setArtistOnlySearch] = useState(false);
  ...
  useEffect(() => {
    const savedAlbums = localStorage.getItem("album-wall");

    if (savedAlbums) {
      try {
        setAlbums(JSON.parse(savedAlbums));
      } catch {
        localStorage.removeItem("album-wall");
      }
    }
  ...
  async function searchAlbums() {
    ...
    setError("Please enter an album name or artist.");
    return;
  }
    ...
    try {
      const response = await fetch(
        `/api/search-album?title=${encodeURIComponent(
          title.trim()
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "MusicBrainz search failed."
        );
      }

      setReleases(data.releases ?? []);
      setArtistOnlySearch(data.artistOnly ?? false);
      setYearFilter("all");
  ...
  async function addAlbum(release: Release) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Unable to get album cover."
        );
      }
  ...
  function toggleAlbum(id: number) {
    setSelectedAlbums((current) =>
      current.includes(id)
        ? current.filter((albumId) => albumId !== id)
        : [...current, id]
    );
  ...
  setWallpaperOrder((currentOrder) => {
    ...
    const firstIndex = newOrder.indexOf(swapSource);
    const secondIndex = newOrder.indexOf(id);
  ...
  ) 
{
  ...
  try {
    const requiredAlbums = gridSize * gridSize;

  // If this is a manual Generate, require enough selected albums.
  // Shuffle passes customOrder, so it does not need selectedAlbums.
  if (
    !customOrder &&
    selectedAlbums.length < requiredAlbums
  ) {
    return;
  }

  let order = customOrder ?? selectedAlbums;
  setWallpaperOrder(order);
  ...
  const selected = order
    ...
    const images = await Promise.all(
      selected.slice(0, gridSize * gridSize).map((album) => {
  ...
  if (albums.length < requiredAlbums) {
    return;
  }
  ...
  const randomAlbums = shuffled
    .slice(0, requiredAlbums)
    .map((album) => album.id);

  setWallpaperOrder(randomAlbums);

  // Generate the shuffled wallpaper immediately
  await generateWallpaper(randomAlbums);
}
...
const filteredReleases =
  yearFilter === "all"
    ...
    : releases.filter((release) =>
        release.date?.startsWith(yearFilter)
      );
  ...
  <>
        ...
        <div className="w-[420px] border-2 border-white border-b-black border-r-black bg-[#c0c0c0] shadow-[4px_4px_0_#000]">
          ...
          <div className="p-5">
            ...
            <p>
              ...
              Select albums and create a collage.
            </p>

            <p>
              <span className="font-bold">4.</span>{" "}
              Shuffle to generate a random collage🪄.
            </p>

          </div>

            <div className="mt-5 border-2 border-[#808080] border-b-white border-r-white bg-[#e0e0e0] p-3 text-xs">
              Your album library is stored locally
              in your browser.
            </div>
    ...
    <header>
      {/* Title Bar */}
        ...
        <div className="flex items-center justify-between bg-[#000080] px-2 py-1 font-[family-name:var(--font-pixelify)] text-white">        <div className="flex items-center gap-2">
      ...
      <div className="flex gap-5 border-b border-[#808080] bg-[#c0c0c0] px-3 py-1 font-[family-name:var(--font-pixelify)] text-sm text-black">        <span>File</span>
        <span>Edit</span>
        <span>View</span>
        <span>Library</span>
        <span>Wallpaper</span>
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              {artistOnlySearch && (
                <select
                  ...
                  className="border-2 border-[#808080] border-r-white border-b-white bg-white px-3 py-1 text-sm"
                >
                  <option value="all">
                    All years
                  </option>
            ...
            <div className="space-y-2">
              ...
              {filteredReleases.map((release) => (
                ...
                >
                  ...
                  {albums.some(
                    (album) => album.releaseId === release.id
                  ) || release.added ? (

                    <button
                      disabled
                      className="font-[family-name:var(--font-pixelify)] border-2 border-[#808080] bg-[#d4d0c8] px-4 py-1.5 text-sm text-[#404040]"
                    >
                      ✓ Added
                    </button>
                    ...
                    <button
                      ...
                      className="font-[family-name:var(--font-pixelify)] border-2 border-white border-b-black border-r-black bg-[#c0c0c0] px-4 py-1.5 text-sm text-black active:border-b-white active:border-r-white disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      {addingId === release.id
                        ? "Adding..."
                        : "Add"}
        ...
        {generating && (
          ...
          <div className="mb-6 flex items-end justify-between">
            ...
            {selectedAlbums.length} albums selected
              ...
              {selectedAlbums.length > gridSize * gridSize && (
                <p className="mt-2 text-sm text-amber-600">
                  You selected{" "}
                  {selectedAlbums.length - gridSize * gridSize} extra{" "}
                  {selectedAlbums.length - gridSize * gridSize === 1
                    ? "album"
                    : "albums"}.
                  Only {gridSize * gridSize} will be used.
                </p>
              )}
          ...
          <button
            ...
            {wallpaperUrl && wallpaperOrder.length >= gridSize * gridSize && (
            <div className="overflow-hidden border-2 border-white bg-[#c0c0c0] p-2 shadow-[2px_2px_0_#808080]">
              ...
              >
                {wallpaperOrder.slice(0, gridSize * gridSize).map((id) => {
                  ...
                  return (
                    <div
                      ...
                      onDoubleClick={() => {
                        setSwapMode(true);
                        setSwapSource(album.id);
                      }}
                      onClick={() => {
                        if (swapMode) {
                          handleSwapClick(album.id);
                        }
                      }}
